...
const VendorFilter = () => {
  ...
  const handleChange = (position: number) => {
   ...
   setVendorIds(checkedState.filter((state:any) => Object.values(state)[0] === true));
  };

  const handleSearch = async (e: { preventDefault: () => void; target: { name: any; value: any; }; }) => {
    e.preventDefault();
      ...
      <div className="categories">
        ...
        <div className="collapse show" id="vendor">
          <div className="filter-search">
            <form action="#">
              <input type="text" name="serach" onChange={(e) => handleSearch(e)} placeholder="Search" className="form-control" />
          ...
          <div className="filter-category">
            <ul className="category-list">
              {
              vendors?.map(({businessName, _id}: any, index: any) =>
                <li key={_id}>
                  <div className="form-check">
                    <label className="form-check-label" htmlFor={`vendorCheck${_id}`}>
                      <span style={{fontSize: '14px'}}>{(businessName).toUpperCase()}</span>
                    </label>
                    <input
